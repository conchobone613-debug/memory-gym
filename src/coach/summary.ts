...
import { calcLadderStatus, currentCalcLevel, ladderSession } from '../calc/ladders';
...
function rungsOf(statuses: RungStatus[], log: CalcLog, sums: SessionSummary[], now: number): CalendarRung[] {
  /* 플래시 판·유형 한정 판은 사다리에 들지 않으므로(calcPracticeIds) 칸 줄의 한 문항 시간·최근 두 판에서도 뺀다 */
  const levelOf = new Map(log.sessions.filter((s) => ladderSession(s.params)).map((s) => [s.id, Number(s.params.level)]));
