...
import { Btn, ConfirmBtn, Field, Panel } from './ui';
...
export default function SyncPanel() {
  ...
  const run = async (c: string) => {
    ...
    try {
      ...
      setMsg(
        `맞췄습니다 — 올린 기록 ${r.pushed}개, 받은 기록 ${r.pulled}개` +
  ...
  const join = async () => {
    ...
    if (!isValidSyncCode(c)) { setMsg('코드가 32자리 형식이 아닙니다. 다시 확인해 주십시오.'); return; }
    await saveSettings({ syncCode: c, lastSyncAt: 0 });
    setEntering(false);
    setEntered('');
    await run(c);
  ...
  const stop = async () => {
    ...
    setMsg('이 기기에서 동기화를 껐습니다. 기록은 그대로 남아 있습니다.');
  };

  const link = code ? `${location.origin}${location.pathname}#/settings?sync=${code}` : '';

  return (
    <Panel title="기기 간 동기화">
      {!code ? (
        <>
          <p className="text-sm text-muted">
            집 PC·진료실 PC·휴대폰에서 같은 기록을 보시려면 여기서 켜십시오. 로그인은 없습니다.
            <b className="text-fg"> 첫 기기에서 '동기화 시작'</b>을 누르시고, 나온 코드를 다른 기기에 넣으시면 됩니다.
          </p>
        ...
        <>
          <div className="grid gap-3 md:grid-cols-2">
            <Field label="이 기기의 동기화 코드" hint="다른 기기에서 '다른 기기 코드 넣기'에 그대로 붙이십시오">
              <input readOnly value={code} onFocus={(e) => e.currentTarget.select()} className="font-mono text-sm" />
            </Field>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <Btn
              disabled={busy}
              onClick={() => { navigator.clipboard?.writeText(code); setMsg('코드를 복사했습니다.'); }}
            >
              코드 복사
            </Btn>
            <ConfirmBtn
              ...
              onConfirm={stop}
            />
            <span className="tnum ml-auto text-xs text-muted">마지막 동기화 {fmt(settings.lastSyncAt)}</span>
          </div>
        </>
      )}
