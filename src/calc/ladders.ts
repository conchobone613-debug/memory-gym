...
/** 이 칸의 연습 세션 id(연습 판은 params.level 로 칸을 가린다) */
export function calcPracticeIds(log: CalcLog, n: number): Set<string> {
  return new Set(log.sessions.filter((s) => s.mode === 'practice' && Number(s.params.level) === n).map((s) => s.id));
}
